<x-wizard-content-container>
    <form wire:submit="save">
        ...
        <div class="form-group">
            ...
            <input type="color" name="theme_color" id="theme_color" class="form-control" wire:model="theme_color" />
            <span class="color-box" id="theme_color_box" style="background-color: {{ $appearanceSettings->theme_color ?? '#FAEED8' }}"></span>
        </div>
        <div class="form-group">
            <label for="primary_color">Primary Color</label>
            <input type="color" name="primary_color" id="primary_color" class="form-control" wire:model="primary_color" />
        </div>
        <div class="form-group">
            <label for="secondary_color">Secondary Color</label>
            <input type="color" name="secondary_color" id="secondary_color" class="form-control" wire:model="secondary_color" />
        </div>
        <div class="form-group">
            <label for="text_color">Text Color</label>
            <input type="color" name="text_color" id="text_color" class="form-control" wire:model="text_color" required>
        </div>
        <div class="form-group">
            <label for="logo">Logo</label>
            <input type="file" name="logo" id="logo" class="form-control">
            @if($appearanceSettings->logo_path)
                <img src="{{ asset('storage/' . $appearanceSettings->logo_path) }}" alt="Logo" class="img-fluid">
            @endif
        </div>
