...
class CertificateOfResidencyRequestProfile extends Component
{
    ...
    {
        $user = auth()->user();
        $this->documentType = DocumentType::CERTIFICATE_OF_RESIDENCY;
        ...
        else
        {
            $this->availableRequesters = $user->resident->household->residents
                ->pluck('first_name', 'middle_name', 'last_name', 'id')
                ->toArray();
        }
    }
    ...
    {
        return view('livewire.documents.certificate-of-residency-request-profile', ['availableRequesters' => $this->availableRequesters]);
    }
}
